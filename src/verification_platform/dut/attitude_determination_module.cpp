...
AttitudeDeterminationModule::AttitudeDeterminationModule(uvm_component_name name, vip_if *in, vip_if *out) : uvm_component(name)
{
    vif_in = in;
    vif_out = out;
}

void AttitudeDeterminationModule::build_phase(uvm_phase &phase)
{
...
void AttitudeDeterminationModule::run_phase(uvm_phase& phase)
{
    ...
    while(true){
        ...
        Sky asky = attitude_determination.process(gsky);
        auto end = high_resolution_clock::now();
        duration<double> elapsed_seconds = end-start;
        asky.time.push_back(elapsed_seconds.count());
