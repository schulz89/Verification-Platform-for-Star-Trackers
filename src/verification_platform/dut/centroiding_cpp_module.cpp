...
centroiding_cpp_module::centroiding_cpp_module(uvm_component_name name, vip_if *in, vip_if *out) : uvm_component(name)
{
    vif_in = in;
    vif_out = out;
    ...
    star_filter = new StarFilterSW(cfg_threshold);
    centroider = new Centroider(cfg_cdpu_count);
}

centroiding_cpp_module::~centroiding_cpp_module()
{
    // delete star_filter;
    delete centroider;
}

void centroiding_cpp_module::build_phase(uvm_phase &phase)
{
...
void centroiding_cpp_module::run_phase(uvm_phase& phase)
{
